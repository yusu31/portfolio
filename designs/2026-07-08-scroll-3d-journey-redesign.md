...
- 新規依存: `@splinetool/react-spline`（未導入・要検証）
- 各セクションの3D造形（サッカーボール的モチーフ、バスケゴール的モチーフ等）はSplineで作成し、
  質感（transmission/ガラス感/発光）を作り込む
- バンドルサイズへの影響を事前に検証してから本導入を決める（軽量なR3Fプリミティブ代替案も残す）
- 「ボールが画面下1/3だけ透明感を出て見える」という質感要求は、Spline側のマテリアル設定 or
...
| 1 | 新規セッション/ブランチで白紙からプロジェクト構造を作成。ScrollControls + 1セクション（Home→Projects間）だけで「スクロールでカメラが進む」感覚を検証 |
| 2 | Spline導入検証（軽量な1オブジェクトで質感テスト、バンドルサイズ確認） |
...
## 10. オープンクエスチョン（実装開始前に決めること）

1. **ルーティング方式**: 1ページ内スクロール（推奨） vs 現行維持の複数ルート。
   複数ルート維持なら各ルートで `<ScrollControls>` を再マウントし、ルート内だけのミニスクロール体験にする折衷案もある
2. **Spline vs R3Fプリミティブ**: Splineの学習コスト・バンドルサイズ次第では、質感強化をR3F側の
   マテリアル調整（transmission/clearcoat等）だけで済ませる縮小版もあり得る
3. **既存リポジトリで作り直すか、新規リポジトリ/ブランチで白紙から始めるか**
   （「別ウィンドウで新規スタート」との整合 — 同一リポジトリ内で新ブランチ + srcを作り直すのか、
   別ディレクトリで試作してから移植するのか）
