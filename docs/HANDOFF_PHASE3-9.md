...
| **Phase A〜E（ボールジャーニー各シーン）** | **Soccer/Basketball/Volleyball 3シーン実装・クリスタル連続性・JourneyEffects** | **#68〜#76** |
| **Phase F（スクロール設計刷新）** | **Lenis廃止→ホイール1回でウェイポイントGSAP移動。lerp廃止。クリスタルジャーニー時0.65倍縮小** | **#78** |
...
### Phase G — ボール物理・軌道のリアリティ向上

**各スポーツのボールの動き・軌道をリサーチして、trajectory ファイルと 3D 演出をリアルに作り直す。**

現状の `*-trajectory.ts` は「動く」だけの仮データ。以下の点が不足している：

| スポーツ | 問題点 | 目標 |
|---|---|---|
| Soccer | ジグザグが単純なX座標往復 | バウンドしながらのドリブル（Y軸に物理バウンス）、シュートの放物線が物理的に正しい弧 |
| Basketball | 放物線の頂点・角度が実際のシュートと乖離 | 3Pシュートの正しい打ち出し角（45〜52°）、バックボード反射も考慮 |
| Volleyball | レシーブ・トス・スパイクのモーションが座標移動のみ | レシーブで低い弾道→トスで高い垂直弾道→スパイクで急激な斜め下降速度 |

**Phase G 開始手順（次セッション）:**

1. `WebSearch` で各スポーツのボール軌道物理をリサーチ（打ち出し角・速度・バウンス係数など）
2. `/brainstorming` で軌道設計を見直す
3. `trajectory.ts` の `parabolaPoint` / `dribbleBounceY` を活用して各trajectory ファイルを更新
4. Crystal の `journeySpeedRef` でスパイク時に高速回転、レシーブ時に減速などの演出を追加

**現在のスクロール方式（Phase F 完了済み）との整合:**
- ウェイポイント数を増やす場合はホットスポットの `hotspotIndex` も見直す
- ウェイポイント間を細かくすれば軌道が滑らか（GSAP 1.8s はウェイポイント間移動なので数を増やすほど細かくなる）
