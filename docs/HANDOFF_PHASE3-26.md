...
**⚠️ 最重要制約: main への push は `.github/workflows/deploy.yml` により Cloudflare Pages へ
自動デプロイされる(= mainマージは即・本番公開)。よって「/scroll-poc → / 昇格」PRを
マージした瞬間に新体験が公開されてしまう。作業順序で以下を厳守:**

1. レスポンシブ・reduced-motion・パフォーマンスを**先に**完了しマージする
   (これらは /scroll-poc 配下の改善なので、本番の `/` は旧サイトのまま安全)
2. ルーティング昇格(+旧コンポーネント削除)PRは**最後に作成し、マージせず保持**
3. Phase 6 のチェックを昇格ブランチ上(ローカル or プレビュー)で実施
4. 全チェック通過+ユーザー最終承認後に昇格PRをマージ → **そのマージが公開**
...
| ルーティング統合 | `/scroll-poc` を `/` に昇格、`App.tsx` の `LegacyChrome` 分岐・旧ルート・旧コンポーネント削除。**PRはマージせず保持** |
...
- [ ] 保持していたルーティング昇格PRをマージ(= main pushでCloudflare Pagesへ自動デプロイ=公開)
...
- 最後に /scroll-poc を / に昇格し、App.tsxのLegacyChrome分岐・旧ルート・旧コンポーネントを削除
- 検証はoffset直指定パターン、goto は wait_until="load"+12秒待ち
- **⚠️ mainマージ=即・本番公開(Actionsで自動デプロイ)。ルーティング昇格PRは
  最後に作成し、Phase 6(公開前チェック)通過+ユーザー承認まで絶対にマージしない**。
  それ以外のPRは /scroll-poc 配下の改善なのでマージしてよい。
