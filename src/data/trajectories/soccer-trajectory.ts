...
export const SOCCER_WAYPOINTS: Waypoint[] = [
  ...
  { progress: 0.00, pos: [ 0, -1.0, +6], camOffset: [0,  0.8, 4.0], rotSpeed: 1.5 },
  // バウンス頂点1
  { progress: 0.03, pos: [ 0, -0.6, +4], camOffset: [0,  0.8, 4.0], rotSpeed: 1.8 },
  // 着地1
  { progress: 0.06, pos: [ 0, -1.0, +2], camOffset: [0,  0.8, 4.0], rotSpeed: 1.5 },
  // バウンス頂点2（中盤越え）
  { progress: 0.09, pos: [ 0, -0.6, -0], camOffset: [0,  0.8, 4.2], rotSpeed: 1.8 },
  // 着地2
  { progress: 0.12, pos: [ 0, -1.0, -2], camOffset: [0,  0.8, 4.0], rotSpeed: 1.5 },
  // バウンス頂点3
  { progress: 0.15, pos: [ 0, -0.6, -4], camOffset: [0,  0.8, 4.2], rotSpeed: 1.8 },
  // 着地3（ジグザグ開始地点）
  { progress: 0.18, pos: [ 0, -1.0, -5], camOffset: [0,  0.8, 4.0], rotSpeed: 1.5 },

  // ── Phase 2: ジグザグ×4（中盤〜アタッキングサード） ───────────────────
  // カメラ: 折り返しの外側に振ってダイナミクス + 前方視野を維持

  // 左への助走
  { progress: 0.22, pos: [-2, -0.7, -7], camOffset: [ 0.5, 0.8, 4.5], rotSpeed: 1.6 },
  // 左折り返し（hotspot 0）─ ゴールから13m地点
  { progress: 0.27, pos: [-4, -1.0, -9], camOffset: [ 1.2, 0.8, 5.0], rotSpeed: 0.4,
    hotspotIndex: 0, impact: true },
  // 右へ切り返し
  { progress: 0.31, pos: [-1, -0.7,-10], camOffset: [-0.3, 0.8, 4.8], rotSpeed: 1.8 },

  // 右への助走
  { progress: 0.35, pos: [ 2, -1.0,-11], camOffset: [-0.8, 0.8, 4.5], rotSpeed: 1.6 },
  // 右折り返し（hotspot 1）─ フィールド中央付近
  { progress: 0.40, pos: [ 4, -1.0,-12], camOffset: [-1.2, 0.8, 5.0], rotSpeed: 0.4,
    hotspotIndex: 1, impact: true },
  // 左へ切り返し
  { progress: 0.44, pos: [ 1, -0.7,-13], camOffset: [ 0.3, 0.8, 4.8], rotSpeed: 1.8 },

  // 左への助走
  { progress: 0.47, pos: [-2, -1.0,-13], camOffset: [ 0.8, 0.8, 4.5], rotSpeed: 1.6 },
  // 左折り返し（hotspot 2）─ アタッキングサード
  { progress: 0.52, pos: [-4, -1.0,-14], camOffset: [ 1.2, 0.8, 5.0], rotSpeed: 0.4,
    hotspotIndex: 2, impact: true },
  // 右へ切り返し
  { progress: 0.55, pos: [-1, -0.7,-14], camOffset: [-0.3, 0.8, 4.8], rotSpeed: 1.8 },

  // 右折り返し（hotspot 3）─ ペナルティエリア内
  { progress: 0.60, pos: [ 3, -1.0,-15], camOffset: [-1.2, 0.8, 5.0], rotSpeed: 0.4,
    hotspotIndex: 3, impact: true },

  // ── Phase 3: ゴール前ロングボール放出 ────────────────────────────────
  // カメラ: 「進行方向を見る」= ボール後方(Z+) からゴール方向(-Z)を向く
  // ゴールへ走り込み（wind-up）
  { progress: 0.64, pos: [ 0, -1.0,-16], camOffset: [0,  0.5, 5.5], rotSpeed: 2.0 },
