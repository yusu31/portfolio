// `/city` のパレット。設計書 §4.1 / §5.3。
//
// **B の `paletteAt(t)` をそのまま移植したもの**で、色の値は1つも変えていない。
// 変えたのは区間解決だけ(`resolveLeg` → `resolveChapter`)。
//
// なぜ「そのまま」でよいか(§4.1):
//   色は `t`(道に沿った位置)の関数なので、**ワープしても色は飛ばない**。
//   `t` 自体は連続だからで、急に変わってもその瞬間はワープ VFX が画面を覆っている。
//   ワープ(offset → t の写像)と色(t の関数)を**互いに直交する軸に分けた**のがこの設計の要点で、
//   2つの実装が干渉しないので B の資産をそのまま使える。
//
// 物の色は**その物自身の位置で1回だけ焼く**(B の設計判断2)。
// 空・フォグ・ライトだけはカメラの位置で毎フレーム引く(数個の uniform なので安い)。
//
// ⚠ 未対応: C の ΔE 規律(7閾値)のマージは PR 4〜5 で入れる(§5.4)。
//   Amber 章のオーバーレイ文字が読めない既知バグもそこで閉じる(§2.1)。

import { CHAPTERS, resolveChapter } from './route'

/** 章ごとのパレット。連続移動なので隣り合う2つが混ざる前提で持つ */
export type Palette = {
  id: string
  /** UIに出す表示名 */
  label: string
  ...
  /** 差し色。看板・自販機・信号など少量だけ */
  accent: string
  /** 輪郭線の色。純黒にすると浮くので、暗い有彩色に寄せる */
  outline: string
  /** 影の色 = 陰面に残る間接光の色。three.js では **ambientLight の色 = 影の色** */
  shadowColor: string
  /** 上方向の間接光(空の照り返し) */
  skyColor: string
  ambientIntensity: number
  /** キーライト。道に沿って進み続けるので、位置ではなく**主役からの相対オフセット**で持つ */
...
/**
 * 4章ぶん。朝 → 昼 → 夕 → 夜と進むので、色の移り変わりがそのまま時間の経過として読める。
 * 境界で切り替わらず**なめらかに混ざる**ので、「道を歩くうちに日が暮れる」ように見える
 */
...
/** 範囲外はクランプ。章の解決が1つズレても色が消えないようにする */
 ...
 * 路面の陰・地面の汚れを**パレットのベース色から導出する**ために使う。
...
/**
 * パレットどうしを混ぜる。**「連続移動」を色の側でも守るための関数**。
 * 章の境界で色が飛ぶと、一本道を進んでいるという前提そのものが壊れる
 */
...
/**
 * パレットを渡し始める位置(章の長さに対する割合)。
 *
 * **章ぜんぶを使って隣へ移すと、章の中央が常に2色の50/50になり、
 * 純粋なパレットがどこにも存在しなくなる**(B が実際に踏んだ失敗)。
 * 「パレットを絞る」が共通原則なので、章の大半は自分の色を保ち、終盤だけで次へ渡す。
 * 章の終わりでちょうど次の色になるので境界は連続したまま
 */
export const PALETTE_BLEND = 0.24

/**
 * 道の位置 `t` におけるパレット。**パレット運用の中心になる関数**。
 * 道を進むと朝の街から夜の街へ**空間的に**移り変わる
 */
export function paletteAt(t: number): Palette {
  const { index, local } = resolveChapter(t)
  const a = getPalette(CHAPTERS[index].paletteIndex)
  const b = getPalette(CHAPTERS[Math.min(index + 1, CHAPTERS.length - 1)].paletteIndex)
