...
describe('終端静止(offsetクランプ)', () => {
  it('PATH_END_OFFSETでのカメラがContact終着の構図にある(チェイスカム実測: pos≈(1.6, 4.0, -236.6))', () => {
    ...
    expect(camera.position.z).toBeGreaterThan(-238)
    expect(camera.position.z).toBeLessThan(-235)
  })
  ...
  it('PATH_END_OFFSETでの視線がボール最終静止点(CONTACT_REST)の方向を向いている', () => {
    ...
    expect(deg(fwd.angleTo(toRest))).toBeLessThan(10)
  })
