// 輪郭線。**参考例4例のうち輪郭線がはっきりあるのは①だけ**なので、
// ①に倣う B でだけ入れる(docs/references/2026-08-02_x-4examples.md の訂正節)。
//
// ■ 方式の選定
// 現行シーンに残っている `journey/DepthOutline.tsx` は深度バッファのスクリーン空間エッジ検出だが、
// あれは**現行シーン固有の事情**(drei `<Cloud>` が depthWrite:false なので法線パスが使えない)に
// 合わせた設計で、B の構成には対応しない。B の街は低ポリの直方体の集合なので、
// **箱の12稜線を解析的に出す**ほうが確実に「はっきりした線」になり、しかも
// スクリーン空間の閾値調整が要らない(遠景で線がノイズにならない)。
// 逆ハル(方式B)は PR #351 で却下済み — 直方体では法線が不連続で角が割れるため。
//
// ■ データと描画を分ける
// A で `groundMarks.ts`(純データ) と `groundTexture.ts`(描画) を分けたのと同じ理由。
 ...
 * 線だけでフレームを落とさないため(密度を上げるのがこの試作の主眼なので、
 * 数を増やす方向に振られる前提で上限を持たせている)
 */
 ...
 *
 * 箱ごとに色を変えられるようにしてあるのは、B では輪郭線の色も
 * その場所のパレットから引くため(夜の区間の線は暗く、朝の区間の線は明るい)
 */
