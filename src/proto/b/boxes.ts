 ...
 * ①でも線が乗っているのは構造物で、細かい散乱物は色の塊として置かれていた
 */
...
/** 主役(人物)の各パーツ。**ローカル座標**で持ち、描画側はグループごと動かす */
export function heroParts(height: number): PlacedBox[] {
  const unit = height / 1.8
  const s = (x: number) => x * unit
  // 背面追従なので画面に映るのは背中。背負い物があるとシルエットが読みやすくなる
  return [
    // 胴
    { t: 0, center: [0, s(1.05), 0], size: [s(0.46), s(0.62), s(0.26)], rotY: 0, color: '#3f4a63', outline: true },
    // 頭
    { t: 0, center: [0, s(1.58), 0], size: [s(0.3), s(0.32), s(0.3)], rotY: 0, color: '#e8c9a8', outline: true },
    // リュック(背中側 = カメラ側)
    { t: 0, center: [0, s(1.08), s(-0.22)], size: [s(0.36), s(0.44), s(0.2)], rotY: 0, color: '#c2553f', outline: true },
    // 腕
    { t: 0, center: [s(-0.32), s(1.0), 0], size: [s(0.14), s(0.56), s(0.16)], rotY: 0, color: '#3f4a63', outline: true },
    { t: 0, center: [s(0.32), s(1.0), 0], size: [s(0.14), s(0.56), s(0.16)], rotY: 0, color: '#3f4a63', outline: true },
    // 脚
    { t: 0, center: [s(-0.13), s(0.37), 0], size: [s(0.18), s(0.74), s(0.2)], rotY: 0, color: '#2b3346', outline: true },
    { t: 0, center: [s(0.13), s(0.37), 0], size: [s(0.18), s(0.74), s(0.2)], rotY: 0, color: '#2b3346', outline: true },
  ]
}
